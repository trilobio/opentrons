...
def run(ctx):
    ...
    p20m = ctx.load_instrument("p20_multi_gen2", "left", tip_racks=[ctx.load_labware("opentrons_96_filtertiprack_20ul","10")])
    ...
    for pipette in [p20m]:#, p300m]:
        ...
        for labware in [pcr96, standard_plate, agar_plate, deep_well, reservoir12, reservoir]:
            pipette.transfer(20, labware.wells_by_name()["A1"].bottom(), labware.wells_by_name()["A1"], new_tip='never')
        pipette.return_tip()
